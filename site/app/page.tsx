...
const VERIFIED_RELEASE = {
  tag: 'v0.0.0-build.7',
  commit: '3cb576e14882a00c39dd10963aef979c72722623',
  installerUrl: 'https://github.com/Ding-Ding-Projects/material-ollama/releases/download/v0.0.0-build.7/OllamaSetup.exe',
  installerSize: '40,215,508 bytes',
  installerSha256: '2decb56559a30836fb287116eb1cad26cdfe66e3a11bf3e350b62a35a3f4a5eb',
...
const FEATURE_CATALOG: Array<{ id: string; title: string; category: string; summary: string; surface: string }> = [
  ...
  { id: 'updates', title: 'Unsigned update transparency', category: 'Publishing', summary: 'The verified Windows release shows its immutable download facts and unsigned warning.', surface: 'Downloads and status' },
...
const ARTICLES = [
  ...
  { id: 'downloads', title: 'Verified downloads and release evidence', summary: 'The Windows installer is linked to the immutable release manifest, exact commit, size, and hash.', category: 'Publishing', commit: VERIFIED_RELEASE.commit },
]
...
function Overview({ state, onTab, onOpenFeature }: { state: SiteState; onTab: (tab: PageId) => void; onOpenFeature: (id: string) => void }) {
  ...
  return <section className="page-panel"><div className="hero-grid"><div><p className="eyebrow">Material Ollama · landing page</p><h1>Local models, clearly explained.</h1><p className="hero-lede">A product-specific landing, documentation, status, and download surface for a local-first desktop companion.</p><div className="boundary"><strong>Important boundary</strong><p>This site introduces the installed desktop application. It is not the primary runtime, it does not host a model, and it is not a playable substitute for the desktop application.</p></div><div className="hero-actions"><button className="button button-primary" type="button" onClick={() => onTab('status')}>View release evidence</button><button className="button button-secondary" type="button" onClick={() => onTab('docs')}>Read documentation</button></div><div className="hero-meta"><span className="status-chip status-good">✅ Release: {VERIFIED_RELEASE.tag} verified</span><span className="status-chip">⌁ State stays in this browser</span></div></div><div className="preview-card" aria-label="Product preview illustration"><div className="preview-chrome"><i /><i /><i /><strong>Material Ollama</strong></div><div className="preview-body"><div className="preview-rail"><b /><b /><b /><b /></div><div className="preview-main"><p className="eyebrow">Local workspace</p><h2>Choose a model, then make it yours.</h2><div className="preview-lines"><i /><i /><i /></div><span className="preview-footer">● Ollama service · ready to inspect</span></div></div></div></div><div className="section-heading"><div><p className="eyebrow">Surface map</p><h2>One honest place to start.</h2></div><p>The landing page names the boundary, documents the product, and offers the verified Windows installer with its release evidence.</p></div><div className="callout callout-info"><strong>Verified release: {VERIFIED_RELEASE.codeName}</strong><p><a className="text-link" href={VERIFIED_RELEASE.installerUrl} target="_blank" rel="noreferrer">Download {VERIFIED_RELEASE.tag} for Windows</a> · {VERIFIED_RELEASE.installerSize} · SHA-256 <code>{VERIFIED_RELEASE.installerSha256}</code>. The installer is unsigned and may trigger an unknown-publisher or SmartScreen warning. <a className="text-link" href={VERIFIED_RELEASE.photoUrl} target="_blank" rel="noreferrer">View the authoritative public dish photo</a>.</p></div><div className="card-grid three-up">{featured.map((feature) => <FeatureCard key={feature.id} feature={feature} onOpen={onOpenFeature} />)}</div><div className="callout callout-info"><strong>{state.schoolMode ? state.schoolName : 'Visitor state'}</strong><p>{state.schoolMode ? 'Focus mode keeps this site in English and suppresses optional playful surfaces until you turn it off.' : 'All settings, notices, search state, and feature selections stay local to this browser. No request is needed to browse the site.'}</p></div></section>
}
...
function Status({ state, notices, onNotice }: { state: SiteState; notices: Notice[]; onNotice: (notice: Omit<Notice, 'id'>) => void }) {
  ...
  return <section className="page-panel"><div className="page-heading"><p className="eyebrow">Status and evidence</p><h1>What is verified, what remains pending.</h1><p>The landing page does not connect to an Ollama service. It reports the independently verified Windows release and keeps the owner-only site boundary explicit.</p></div><div className="status-grid"><div className="status-card"><span className="status-label">Release</span><strong className="status-value good">✅ {VERIFIED_RELEASE.tag}</strong><p>Verified against commit <code>{VERIFIED_RELEASE.commit}</code>.</p></div><div className="status-card"><span className="status-label">Installer</span><strong className="status-value good">● Download verified</strong><p>{VERIFIED_RELEASE.installerSize} · SHA-256 <code>{VERIFIED_RELEASE.installerSha256}</code>.</p></div><div className="status-card"><span className="status-label">Landing access</span><strong className="status-value good">● Owner-only</strong><p><a className="text-link" href={VERIFIED_RELEASE.landingUrl} target="_blank" rel="noreferrer">Open the verified landing URL</a>; anonymous access is intentionally bounded.</p></div><div className="status-card"><span className="status-label">Heartbeat</span><strong className="status-value">{heartbeat}</strong><p>Rendered locally when this page is open.</p></div></div><div className="callout callout-info"><strong>{VERIFIED_RELEASE.codeName}</strong><p><a className="button button-primary" href={VERIFIED_RELEASE.installerUrl} target="_blank" rel="noreferrer">Download Windows installer</a></p><p>The installer is unsigned and may trigger an unknown-publisher or SmartScreen warning. The authoritative public image is <a className="text-link" href={VERIFIED_RELEASE.photoUrl} target="_blank" rel="noreferrer">the catalog photo</a>; no duplicate image is copied into this project.</p></div><div className="evidence-table"><div className="evidence-head"><span>Evidence item</span><span>State</span><span>Meaning</span></div>{[['Landing source', 'Available', 'The site source is present and has a dedicated build entry point.'], ['Desktop installer', 'Verified', `Immutable ${VERIFIED_RELEASE.tag} asset with a verified size and SHA-256.`], ['Release metadata', 'Verified', `${VERIFIED_RELEASE.codeName} is linked to the public catalog image.`], ['Ollama service', 'Not connected', 'The site never pretends to inspect a machine-local service.'], ['Capture matrix', 'Pending', 'Real built-artifact captures are a separate delivery responsibility.']].map(([name, stateValue, meaning]) => <div className="evidence-row" key={name}><span data-label="Evidence item">{name}</span><span data-label="State" className={stateValue === 'Available' || stateValue === 'Verified' ? 'good' : 'pending'}>{stateValue}</span><span data-label="Meaning">{meaning}</span></div>)}</div><div className="inline-actions"><button className="button button-secondary" type="button" onClick={() => onNotice({ tone: 'info', title: 'No live service connected', detail: 'This landing surface keeps the service boundary honest.' })}>Create local status notice</button>{notices.length > 0 && <span className="field-help">{notices.length} notice{notices.length === 1 ? '' : 's'} in the local history.</span>}</div></section>
}
