...
**A known, narrow, environment-specific false positive**: on a CRLF checkout (this one included), the first test — `the committed collection is byte-identical to what the generator produces from app/ui/ui.go right now` — currently fails, because `scripts/generate-postman-collection.mjs` always writes the file with `\n` line endings while Git checked the committed file out with `\r\n`. This was confirmed to be an EOL artifact rather than real drift: normalizing both the committed file and a freshly-regenerated copy to `\n` and comparing them byte-for-byte shows they are identical (94,690 bytes with CRLF vs. 91,717 bytes with LF — exactly the 2,973-line difference `\r\n` vs. `\n` would produce). The other three tests, which parse both sides as JSON rather than comparing raw bytes, are unaffected by line endings and pass cleanly. This is why this row's inventory status is `in-progress` rather than `verified` with its `focusedCheck` pointed at the EOL-independent route-count test instead of the fragile byte-identity one — the collection content itself is current, but the exact-byte check the generator ships needs to normalize line endings before it can be trusted on every checkout.
