...
func TestWebSearchResponsesWriterNonStreamingLoopExhaustionReturnsToolResult(t *testing.T) {
	...
	if searches != maxWebSearchLoops || followUps != maxWebSearchLoops+1 || response.Status != "completed" || response.Output[len(response.Output)-1].Content[0].Text != "answer from existing results" {
